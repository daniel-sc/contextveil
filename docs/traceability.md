...
| SET-007 | Selection defaults apply independently to each source | src/setup/enrollment.rs; src/setup/mod.rs (`automatic_item_for`, `annotate_collisions`) | tests/setup.rs::collision_defaults_apply_independently_within_a_visual_group | covered |
...
| SET-016 | Equal-value groups are visual only and every source remains independently selectable | src/setup/enrollment.rs; src/setup/mod.rs; src/setup/render.rs | tests/setup.rs::equal_value_groups_have_independent_member_selection, ::manual_addition_preserves_existing_display_order | covered |
