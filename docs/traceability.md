...
| SET-002 | Setup auto-inspects the process environment for vocabulary, URL, and Known Source candidates | src/setup/mod.rs (`environment_candidates`) implements vocabulary and URL inspection; Known Source inspection is pending | tests/setup.rs::a_gated_environment_candidate_is_enrolled_by_default, ::a_database_url_candidate_is_enrolled_as_its_environment_source; issue #11 covers the remaining path | gap |
...
| SET-006 | Vocabulary remains the default gate, with bounded URL and Known Source exceptions | src/setup/vocabulary.rs; src/setup/credential_url.rs; Known Source exception is pending | vocabulary.rs unit tests; credential_url.rs positive/negative fixtures; tests/setup.rs::non_credential_url_shapes_do_not_bypass_name_gating | gap |
...
| TST-003 | Filesystem tests include Known Source paths and grouped collision exclusions in addition to existing coverage | Grouped collision filesystem coverage is implemented; Known Source paths remain pending under issue #11 | tests/setup.rs grouped source/collision/wildcard cases; tests/diagnose.rs::doctor_groups_aliases_and_excludes_all_of_their_source_files | gap |
...
pending under `limitations.md` DEV-003. Issue #11 must close those rows before
the Known Source discovery claims ship.
