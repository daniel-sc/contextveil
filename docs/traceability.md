...
| SET-006 | Secret-like name Known Source Rule uses the exact maintained vocabulary and no independent format/entropy/length admission | src/setup/vocabulary.rs; src/setup/mod.rs (`build_items`, `rank_of`) | vocabulary.rs unit tests; setup name-gating fixtures | covered |
...
| SET-016 | Equal current values form phase-local Candidate Groups with all-alias selection, wildcard handling, deduplicated rule names, one fixed admission weight, and stable canonical order | src/setup/mod.rs (`merge_item`, grouped `Item`, `rank_of`, `update_suppression`, `selected_sources`) | tests/setup.rs::equal_environment_candidates_are_one_group_and_enroll_every_alias, ::equal_values_in_different_phases_remain_separate_choices, ::several_matching_rules_contribute_one_admission_weight, ::known_sources_persist_explicit_refs_and_bypass_name_gating, ::project_known_source_aliases_form_one_candidate_group, ::a_partially_enrolled_group_saves_all_aliases_but_skip_is_exact, ::resolvable_manual_sources_merge_into_an_existing_group, ::a_selected_wildcard_suppresses_redundant_keyed_candidates, ::aliases_split_into_separate_rows_after_their_values_diverge | covered |
