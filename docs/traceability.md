...
| SET-002 | Setup applies every applicable Known Source Rule independently of adapter selection, then applies the centralized Common Literal and simple-reference exclusion | Candidate discovery and centralized `excluded_automatic_value` predicate run before integration selection in src/setup/mod.rs | Existing name, URL, additive-location, bounded-probe, and `tests/setup.rs::simple_reference_values_are_excluded_from_every_automatic_source_family` | covered |
...
| SET-011 | Collision analysis searches bounded textual regions with one byte-exact multi-pattern pass and excludes every whole equal-value alias file | src/setup/collision.rs (`Subject::source_files`, `analyze`, `scan_file`) | collision.rs binary-region, buffer-boundary, size-boundary, overlapping-pattern, and alias-exclusion tests; setup alias/source-family fixtures | covered |
| SET-012 | Collision output shows counts and sanitized filenames only, never values/snippets or skipped files | src/setup/collision.rs (`Collisions::describe`) | collision.rs::reports_contain_filenames_and_counts_but_never_values, ::filenames_are_sanitized_for_the_terminal, ::the_file_size_limit_is_inclusive_and_oversized_files_are_skipped | covered |
| SET-013 | Unavailable non-enrolled files excluded without aborting discovery; enrolled malformed sources must be repaired or removed | src/setup/discovery.rs (`inspect`, `inspect_npmrc`); src/setup/mod.rs (blocking on enrolled malformed sources) | discovery.rs malformed, unreadable, and non-UTF-8 fixtures; tests/setup.rs::an_enrolled_malformed_source_must_be_repaired_or_removed, ::an_unavailable_discovered_file_does_not_stop_discovery, ::malformed_and_non_utf8_known_sources_are_visible_and_secret_safe | covered |
...
| SET-016 | Equal-value admitted aliases form one selectable enrollment unit and selected groups persist every represented source | src/setup/enrollment.rs; src/setup/mod.rs; src/setup/render.rs | tests/setup.rs::equal_value_group_toggle_applies_to_every_alias, ::resolvable_manual_sources_merge_into_an_existing_group, ::excluded_automatic_aliases_do_not_join_an_existing_enrollment | covered |
| SET-017 | Credential-bearing URL values already surfaced by bounded discovery become whole-value eligible sources without recursive scanning, subject to SET-023 | src/setup/credential_url.rs; src/setup/mod.rs; src/setup/known_source.rs | credential_url.rs fixtures; setup env/dotenv/properties URL fixtures | covered |
| SET-018 | Known Source Rules identify automatic eligibility, run independently of adapters, compose before centralized exclusion, and persist explicit references | src/setup/mod.rs, src/setup/known_source.rs, and src/setup/discovery.rs; no runtime rule variant exists | known_source.rs probe fixtures; tests/setup.rs::known_sources_persist_explicit_refs_and_bypass_name_gating, ::common_literals_are_excluded_from_every_automatic_source_family | covered |
| SET-019 | Recognized credential documents use JSON5 and bounded permissive probes; non-empty listed strings are independently eligible subject to SET-023 | src/setup/known_source.rs declarative probes; centralized exclusion in src/setup/mod.rs | known_source.rs bounded-probe fixtures; tests/setup.rs::common_literals_are_excluded_from_every_automatic_source_family | covered |
...
| SET-023 | Wholly new automatic sources resolving to an exact Common Literal or complete simple variable reference are silently excluded after rule composition and before grouping; existing, manual, wildcard, and runtime behavior bypass the exclusion | src/setup/vocabulary.rs (`excluded_automatic_value`); src/setup/mod.rs (`merge_automatic_item`) | vocabulary.rs Common Literal and simple-reference grammar tests; tests/setup.rs::simple_reference_values_are_excluded_from_every_automatic_source_family, ::excluded_automatic_aliases_do_not_join_an_existing_enrollment, and existing Common Literal/manual/wildcard fixtures | covered |
...
| ID | Requirement | Implementation | Evidence | Status |
| --- | --- | --- | --- | --- |
| REG-001 | Every non-empty normalized value from an enrolled source, including Common Literals, simple variable references, and wildcard values, is an exact runtime pattern | src/source.rs; src/matcher.rs (`Redactor::new`) | Source and matcher exactness fixtures; tests/setup.rs::manual_and_wildcard_enrollment_can_protect_excluded_values | covered |
...
| ID | Requirement | Implementation | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-001 | Detect all four harnesses; present detected or already-managed harnesses; Claude selected by default; experimental integrations unselected unless already ContextVeil-installed | src/setup/integrations.rs (visibility and default-selection logic); src/integration/mod.rs (`Tier`) | src/integration/mod.rs::only_claude_is_production; tests/setup.rs::an_experimental_integration_requires_an_affirmative_choice; tests/setup.rs::an_undetected_harness_is_not_offered | covered |
| INT-002 | Explain how to make a coding agent detectable when no harnesses are offered | src/setup/integrations.rs (`render_rows`, visibility filter) | tests/setup.rs::an_undetected_harness_is_not_offered | covered |
