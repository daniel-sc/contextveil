...
| REL-002 | Maintained install script: detects platform/arch, downloads, verifies checksum, atomically installs, overridable default destination | install.sh (`--install-dir`, `--version`, `--allow-major-upgrade`; platform detection, checksum verification) | scripts/release-check.sh (clean-install, `--install-dir`, unknown-option rejection cases) | covered |
