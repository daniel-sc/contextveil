...
| --- | --- | --- | --- | --- |
| SEC-001 | Help prevent resolved values reaching model context via covered adapter paths | src/matcher.rs (`Redactor`), src/redact.rs, src/adapter/{claude,codex,copilot,opencode}.rs | tests/leaks.rs::every_adapter_redacts_an_enrolled_json_value; per-adapter protocol fixtures in tests/claude_hook.rs, tests/codex_hook.rs, tests/copilot_hook.rs, tests/opencode/plugin.test.ts | covered |
...
| SET-002 | Setup applies every applicable Known Source Rule independently of adapter selection | Candidate discovery runs before integration selection in src/setup/mod.rs; environment and recognized-store discovery do not inspect selected adapters | Existing name, URL, override, and recognized-schema setup fixtures | covered |
| SET-003 | Recursive project dotenv discovery with documented exclusions; no symlinks/special files | src/setup/discovery.rs (`project_files`, `walk`) | discovery.rs::discovery_is_recursive_and_includes_untracked_files, ::excluded_directories_are_never_entered, ::symlinks_and_special_files_are_skipped, ::a_fifo_named_like_a_dotenv_file_is_never_read, ::non_utf8_paths_are_reported_as_unavailable | covered |
...
| SET-007 | Automatic candidates selected by default unless colliding; unresolved automatic, enrolled, and manual choices stay selected | src/setup/mod.rs (`automatic_item_for`, `annotate_collisions`, grouped selection state) | tests/setup.rs::an_unresolved_automatic_candidate_remains_selected, ::a_colliding_candidate_is_visible_but_unselected, ::a_manual_collision_warns_without_reversing_selection, ::a_partially_enrolled_group_saves_all_aliases_but_skip_is_exact | covered |
...
| SET-016 | Equal current values form phase-local Candidate Groups; least Source Identity representatives, two-tier initial rows, fixed rule-name attribution, manual-loop stability, and save normalization determine ordering without rank | src/secret.rs (`SourceId::cmp`); src/setup/mod.rs (`merge_item`, `sort_initial_items`, `update_suppression`, `selected_sources`) | tests/setup.rs grouping/wildcard fixtures plus ::initial_rows_use_existing_then_new_tiers_but_save_uses_identity_order, ::save_normalizes_source_kinds_and_normalized_identity_fields, ::group_representatives_ignore_discovery_iteration_order, ::manual_aliases_keep_current_member_order_then_save_by_identity | covered |
...
| SET-018 | Canonical Known Source Rules are automatic admission only, run independently of adapters, and recognized-store rules persist explicit references from bounded paths | src/setup/mod.rs, src/setup/known_source.rs, and src/setup/discovery.rs; no runtime rule variant exists in src/source.rs | Existing explicit-reference, override, bounded-path, and integration-phase ordering fixtures | covered |
| SET-019 | Recognized JSON source documents use full JSON5; valid unmatched schemas are silent; malformed sources are unavailable; fields are exact and pointer-representable | src/setup/known_source.rs delegates source documents to src/json.rs and retains exact per-store field selection | Existing silent-no-match, malformed, pointer-representability, and JSON5 schema fixtures | covered |
| SET-020 | Supported store schema-family rules cover the exact inventory, including comment-bearing Copilot configuration, while planned rows remain unscanned | src/setup/known_source.rs contains only the supported Codex, OpenCode, Copilot, and Claude bounded schemas; no npmrc or INI resolver exists | Four host filesystem/schema matrices plus known_source.rs::copilot_comment_bearing_config_is_inspected_as_json5; absence of npmrc/INI discovery confirms planned rows are not scanned | covered |
...
| RED-008 | Intervention metadata carries counts/labels only, never values/hashes/content | src/matcher.rs (`Intervention`, `intervention`) | src/matcher.rs::intervention_summary_is_canary_free, ::unsafe_labels_are_aggregated_without_names; tests/leaks.rs::every_adapter_redacts_an_enrolled_json_value, ::diagnostics_never_disclose_an_enrolled_value | covered |
...
| TST-003 | Filesystem tests include Known Source exact/anchored paths, traversal and symlink rules, grouped rule-name deduplication, and grouped collision exclusions | src/setup/discovery.rs and src/setup/known_source.rs filesystem test modules; tests/setup.rs end-to-end setup fixtures | discovery.rs::one_project_walk_collects_only_anchored_known_source_json, ::excluded_directories_are_never_entered, ::symlinks_and_special_files_are_skipped; known_source.rs four independently named host filesystem matrices, ::exact_machine_fifo_and_symlink_to_fifo_are_skipped_promptly, ::copilot_mcp_oauth_directory_symlink_is_not_traversed; tests/setup.rs::known_sources_persist_explicit_refs_and_bypass_name_gating, ::project_known_source_aliases_form_one_candidate_group, ::a_known_source_group_with_an_external_collision_defaults_unselected; tests/diagnose.rs::doctor_groups_aliases_and_excludes_all_of_their_source_files | covered |
...
| TST-006 | Fuzz targets distinguish untrusted JSON5 source documents from strict protocol JSON and cover matcher/TOML/dotenv input | src/fuzz.rs::json_source uses src/json.rs; separate adapter targets continue through strict protocol parsers | src/bin/fuzz_smoke.rs target table, scripts/fuzz-smoke.sh, mise `fuzz-smoke`, and the protocol regression corpora | covered |
